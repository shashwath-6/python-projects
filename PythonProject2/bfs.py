...
class TrafficSimulatorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("AI Traffic Navigation System")
        ...
        self.get_user_input()  # Get start and goal from user
    ...
    def get_user_input(self):
        """Prompts the user for start and goal locations."""
        locations = list(self.locations.keys())
        self.start = simpledialog.askstring("Start Location", f"Enter start location ({', '.join(locations)}):")
        self.goal = simpledialog.askstring("Goal Location", f"Enter goal location ({', '.join(locations)}):")

        if self.start not in self.locations or self.goal not in self.locations:
            messagebox.showerror("Error", "Invalid location! Please restart and enter valid locations.")
            self.root.quit()
        else:
    ...
    def find_and_draw_path(self):
        ...
        if path:
            ...
            messagebox.showinfo("Shortest Path", f"Fastest Route: {' -> '.join(path)}\nTotal Cost: {cost}")
        else:
            messagebox.showerror("No Path Found", "No valid path between selected locations.")
